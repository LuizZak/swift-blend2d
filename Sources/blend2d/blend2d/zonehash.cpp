...
static const BLPrimeNumber blPrimeArray[] = {
...
static const uint8_t blPrimeShift[] = {
...
// ============================================================================
// [BLZoneHashMap - Rehash]
// ============================================================================

void BLZoneHashBase::_rehash(uint32_t primeIndex) noexcept {
  BL_ASSERT(primeIndex < BL_ARRAY_SIZE(blPrimeArray));

  uint32_t newCount = blPrimeArray[primeIndex].prime;
  BLZoneHashNode** newData = reinterpret_cast<BLZoneHashNode**>(calloc(newCount, sizeof(BLZoneHashNode*)));

  // We can still store nodes into the table, but it will degrade.
  if (BL_UNLIKELY(newData == nullptr))
    return;

  BLZoneHashNode** oldData = _data;
  ...
  _rcpValue = blPrimeArray[primeIndex].rcp;
  _rcpShift = blPrimeShift[primeIndex];
  _primeIndex = uint8_t(primeIndex);

  for (uint32_t i = 0; i < oldCount; i++) {
    BLZoneHashNode* node = oldData[i];
    while (node) {
      BLZoneHashNode* next = node->_hashNext;
...
// ============================================================================
// [BLZoneHashMap - Insert / Remove]
// ============================================================================

void BLZoneHashBase::_insert(BLZoneHashNode* node) noexcept {
  uint32_t hashMod = _calcMod(node->_hashCode);
  BLZoneHashNode* next = _data[hashMod];

  node->_hashNext = next;
  _data[hashMod] = node;

  if (++_size > _bucketGrow) {
    uint32_t primeIndex = blMin<uint32_t>(_primeIndex + 2, BL_ARRAY_SIZE(blPrimeArray) - 1);
    if (primeIndex > _primeIndex)
      _rehash(primeIndex);
  }
}

bool BLZoneHashBase::_remove(BLZoneHashNode* node) noexcept {
  uint32_t hashMod = _calcMod(node->_hashCode);

  BLZoneHashNode** pPrev = &_data[hashMod];
  BLZoneHashNode* p = *pPrev;
...
#if defined(BL_TEST)
struct MyHashNode : public BLZoneHashNode {
