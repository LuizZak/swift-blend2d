  ...
  if (isBlitA()) {
    ...
    if (extendX() == BL_PIPE_EXTEND_MODE_ROR) {
      ...
      if (maxPixels() >= 4) {
        ...
        pc->v_swizzle_i32(f->xInc4, f->xSet4, x86::Predicate::shuf(3, 3, 3, 3));
    ...
    if (isPatternF()) {
      ...
      if (isAlphaFetch()) {
        if (isPatternFy()) {
          pc->v_swizzle_lo_i16(f->wd_wb, weights, x86::Predicate::shuf(3, 1, 3, 1));
          if (maxPixels() >= 4)
            pc->v_swizzle_i32(f->wd_wb, f->wd_wb, x86::Predicate::shuf(1, 0, 1, 0));
        }
        else if (isPatternFx()) {
          pc->v_swizzle_i32(f->wc_wd, weights, x86::Predicate::shuf(3, 3, 3, 3));
        }
        else {
          pc->v_swizzle_lo_i16(f->wa_wc, weights, x86::Predicate::shuf(2, 0, 2, 0));
          pc->v_swizzle_lo_i16(f->wb_wd, weights, x86::Predicate::shuf(3, 1, 3, 1));
          if (maxPixels() >= 4) {
            pc->v_swizzle_i32(f->wa_wc, f->wa_wc, x86::Predicate::shuf(1, 0, 1, 0));
            pc->v_swizzle_i32(f->wb_wd, f->wb_wd, x86::Predicate::shuf(1, 0, 1, 0));
          }
        }
      }
      else {
        ...
        if (isPatternFy()) {
          pc->v_swizzle_i32(f->wb_wb, weights, x86::Predicate::shuf(1, 1, 1, 1));
          pc->v_swizzle_i32(f->wd_wd, weights, x86::Predicate::shuf(3, 3, 3, 3));
        }
        else if (isPatternFx()) {
          pc->v_swizzle_i32(f->wc_wd, weights, x86::Predicate::shuf(2, 2, 3, 3));
        }
        else {
          pc->v_swizzle_i32(f->wa_wb, weights, x86::Predicate::shuf(0, 0, 1, 1));
          pc->v_swizzle_i32(f->wc_wd, weights, x86::Predicate::shuf(2, 2, 3, 3));
        }
      }
    }
    ...
    if (isPatternFx()) {
      if (isAlphaFetch()) {
        ...
        if (pc->hasSSE4_1()) {
          ...
          pc->v_swizzle_i32(pixL, pixL, x86::Predicate::shuf(0, 0, 0, 0));
          pc->vmovu8u16(pixL, pixL);
        }
    ...
    if (isPatternFxFy()) {
      if (isAlphaFetch()) {
        ...
        if (pc->hasSSE4_1()) {
          ...
          pc->v_swizzle_i32(pixT, pixT, x86::Predicate::shuf(0, 0, 0, 0));
          pc->v_swizzle_i32(pixL, pixL, x86::Predicate::shuf(0, 0, 0, 0));
      ...
      if (isPatternFx()) {
        ...
        if (isAlphaFetch()) {
          ...
          if (pc->hasSSE4_1()) {
            ...
            pc->v_swizzle_i32(pix0, pix0, x86::Predicate::shuf(0, 0, 0, 0));
            pc->v_load_i32(pix1, m);
            cc->inc(f->x);
            pc->v_swizzle_i32(pix1, pix1, x86::Predicate::shuf(0, 0, 0, 0));
            pc->vmovu8u16(pix0, pix0);
            cc->cmp(f->x, f->w);
            pc->vmovu8u16(pix1, pix1);
            cc->cmovbe(idx.r32(), f->x);
            ...
            pc->v_swizzle_i32(pix2, pix2, x86::Predicate::shuf(0, 0, 0, 0));
            pc->v_combine_hl_i64(pixT, pixL, pix1);
      ...
      if (isPatternFxFy()) {
        ...
        if (isAlphaFetch()) {
          ...
          if (pc->hasSSE4_1()) {
            ...
            pc->v_swizzle_i32(pix0 , pix0 , x86::Predicate::shuf(0, 0, 0, 0));
            pc->v_swizzle_i32(pix0t, pix0t, x86::Predicate::shuf(0, 0, 0, 0));

            pc->v_load_i32(pix1, mA);
            pc->v_load_i32(pix1t, mB);
            cc->inc(f->x);
            pc->v_swizzle_i32(pix1 , pix1 , x86::Predicate::shuf(0, 0, 0, 0));
            pc->v_swizzle_i32(pix1t, pix1t, x86::Predicate::shuf(0, 0, 0, 0));
            pc->vmovu8u16(pix0, pix0);
            ...
            pc->v_swizzle_i32(pix2 , pix2 , x86::Predicate::shuf(0, 0, 0, 0));
            pc->v_swizzle_i32(pix2t, pix2t, x86::Predicate::shuf(0, 0, 0, 0));
            pc->v_combine_hl_i64(pixT, pixL, pix1);
        ...
        if (flags & Pixel::kPC) {
          ...
          switch (format()) {
            ...
            case BL_FORMAT_A8: {
              ...
              pc->v_swizzle_i32(uc1, uc0, x86::Predicate::shuf(3, 3, 2, 2));
              pc->v_swizzle_i32(uc0, uc0, x86::Predicate::shuf(1, 1, 0, 0));
              break;
            }
          }
        }
...
void FetchAffinePatternPart::_initPart(x86::Gp& x, x86::Gp& y) noexcept {
  ...
  pc->v_swizzle_i32(f->tx_ty, f->tx_ty, x86::Predicate::shuf(1, 0, 1, 0));
  ...
  if (isOptimized()) {
    ...
    pc->v_swizzle_i32(f->maxx_maxy, f->minx_miny, x86::Predicate::shuf(1, 1, 1, 1)); // [MaxY|MaxX|MaxY|MaxX|MaxY|MaxX|MaxY|MaxX]
    pc->v_swizzle_i32(f->minx_miny, f->minx_miny, x86::Predicate::shuf(0, 0, 0, 0)); // [MinY|MinX|MinY|MinX|MinY|MinX|MinY|MinX]
  }
  else {
    pc->v_swizzle_i32(f->maxx_maxy, f->minx_miny, x86::Predicate::shuf(3, 3, 2, 2)); // [MaxY|MaxY|MaxX|MaxX]
    pc->v_swizzle_i32(f->minx_miny, f->minx_miny, x86::Predicate::shuf(1, 1, 0, 0)); // [MinY|MinY|MinX|MinX]
    pc->v_swizzle_i32(f->corx_cory, f->corx_cory, x86::Predicate::shuf(1, 1, 0, 0)); // [CorY|CorY|CorX|CorX]
  }

  // vAddrMul.
  if (isOptimized()) {
    pc->v_load_i32(f->vAddrMul, x86::ptr(pc->_fetchData, REL_PATTERN(affine.addrMul)));
    pc->v_swizzle_i32(f->vAddrMul, f->vAddrMul, x86::Predicate::shuf(0, 0, 0, 0));
  }
...
void FetchAffinePatternPart::startAtX(x86::Gp& x) noexcept {
  if (isRectFill()) {
    ...
    pc->v_swizzle_i32(f->px_py, f->px_py, x86::Predicate::shuf(1, 0, 1, 0));
...
void FetchAffinePatternPart::advancePxPy(x86::Xmm& px_py, const x86::Gp& i) noexcept {
  ...
  pc->v_swizzle_i32(t, t, x86::Predicate::shuf(1, 0, 1, 0));
...
void FetchAffinePatternPart::prefetch1() noexcept {
  ...
  switch (fetchType()) {
    ...
    case BL_PIPE_FETCH_TYPE_PATTERN_AFFINE_NN_OPT: {
      pc->v_swizzle_i32(vIdx, f->px_py, x86::Predicate::shuf(3, 1, 3, 1));
  ...
  switch (fetchType()) {
    ...
    case BL_PIPE_FETCH_TYPE_PATTERN_AFFINE_BI_ANY: {
      if (isAlphaFetch()) {
        ...
        pc->v_swizzle_i32(vIdx, f->px_py, x86::Predicate::shuf(3, 3, 1, 1));
        pc->v_sub_i32(vIdx, vIdx, pc->constAsMem(blCommonTable.i128_FFFFFFFF00000000));

        pc->v_swizzle_lo_i16(vWeights, f->px_py, x86::Predicate::shuf(1, 1, 1, 1));
        ...
        pc->v_cmp_gt_i32(vMsk, f->px_py, f->ox_oy);
        pc->v_swizzle_hi_i16(vWeights, vWeights, x86::Predicate::shuf(1, 1, 1, 1));
...
void FetchAffinePatternPart::prefetchN() noexcept {
  switch (fetchType()) {
    case BL_PIPE_FETCH_TYPE_PATTERN_AFFINE_NN_OPT: {
      ...
      pc->v_shuffle_i32(vIdx, f->px_py, f->qx_qy, x86::Predicate::shuf(3, 1, 3, 1));
      ...
      pc->v_shuffle_i32(vMsk0, f->px_py, f->qx_qy, x86::Predicate::shuf(3, 1, 3, 1));
...
void FetchAffinePatternPart::fetch4(Pixel& p, uint32_t flags) noexcept {
  ...
  switch (fetchType()) {
    case BL_PIPE_FETCH_TYPE_PATTERN_AFFINE_NN_ANY: {
      ...
      pc->v_shuffle_i32(vIdx0, f->px_py, f->qx_qy, x86::Predicate::shuf(3, 1, 3, 1));
      pc->v_add_i64(f->px_py, f->px_py, f->xx2_xy2);
      ...
      pc->v_shuffle_i32(vIdx1, f->px_py, f->qx_qy, x86::Predicate::shuf(3, 1, 3, 1));
