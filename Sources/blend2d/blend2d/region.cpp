...
static BLResult blRegionCombineInternal(
  ...
  uint32_t booleanOp, bool memOverlap) noexcept {

  // Make sure the inputs are as required.
  BL_ASSUME(booleanOp != BL_BOOLEAN_OP_COPY);
  BL_ASSUME(booleanOp < BL_BOOLEAN_OP_COUNT);
  BL_ASSUME(aSize > 0);
  BL_ASSUME(bSize > 0);
...
BLResult blRegionCombine(BLRegionCore* self, const BLRegionCore* a, const BLRegionCore* b, uint32_t booleanOp) noexcept {
  const BLInternalRegionImpl* aI = blInternalCast(a->impl);
  const BLInternalRegionImpl* bI = blInternalCast(b->impl);

  if (BL_UNLIKELY(booleanOp >= BL_BOOLEAN_OP_COUNT))
    return blTraceError(BL_ERROR_INVALID_VALUE);

  if (BL_UNLIKELY(aI == bI)) {
    if (booleanOp == BL_BOOLEAN_OP_COPY || booleanOp == BL_BOOLEAN_OP_AND || booleanOp == BL_BOOLEAN_OP_OR)
  ...
  if (aSize <= 1) {
    ...
    return blRegionCombineBR(self, &aBox, b, booleanOp);
  }

  if (bSize <= 1) {
    BLBoxI bBox(bI->boundingBox);
    return blRegionCombineRB(self, a, &bBox, booleanOp);
  }
  ...
  return blRegionCombineInternal(self, aI->data, aSize, aI->boundingBox, bI->data, bSize, bI->boundingBox, booleanOp, self->impl == aI || self->impl == bI);
}

BLResult blRegionCombineRB(BLRegionCore* self, const BLRegionCore* a, const BLBoxI* b, uint32_t booleanOp) noexcept {
  BLInternalRegionImpl* aI = blInternalCast(a->impl);

  // Fast-path.
  if (aI->size <= 1)
    return blRegionCombineBB(self, &aI->boundingBox, b, booleanOp);
  ...
  return blRegionCombineInternal(self, aI->data, aI->size, aI->boundingBox, &bBox, 1, bBox, booleanOp, self->impl == aI);
}

BLResult blRegionCombineBR(BLRegionCore* self, const BLBoxI* a, const BLRegionCore* b, uint32_t booleanOp) noexcept {
  const BLInternalRegionImpl* bI = blInternalCast(b->impl);

  // Box-Box is faster than Box-Region so use this fast-path when possible.
  if (bI->size <= 1) {
    BLBoxI bBox(bI->boundingBox);
    return blRegionCombineBB(self, a, &bBox, booleanOp);
  }
  ...
  return blRegionCombineInternal(self, &aBox, 1, aBox, bI->data, bI->size, bI->boundingBox, booleanOp, self->impl == bI);
}

BLResult blRegionCombineBB(BLRegionCore* self, const BLBoxI* a, const BLBoxI* b, uint32_t booleanOp) noexcept {
  if (BL_UNLIKELY(booleanOp >= BL_BOOLEAN_OP_COUNT))
    return blTraceError(BL_ERROR_INVALID_VALUE);
  ...
  if (booleanOp == BL_BOOLEAN_OP_COPY) {
Copy:
    if (!blIsValid(*b))
      goto Clear;
  ...
  if (booleanOp == BL_BOOLEAN_OP_AND) {
  ...
  if (booleanOp == BL_BOOLEAN_OP_SUB) {
    // case 1, 2.
  ...
  if (booleanOp == BL_BOOLEAN_OP_XOR) {
    if (blIntersectBoxes(box[3], *a, *b)) {
      // Case 7, 8.
