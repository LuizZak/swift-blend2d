...
from utils.data.swift_decls import SourceLocation, SwiftDecl, SwiftEnumDecl, SwiftEnumCaseDecl
...
class SwiftDeclGenerator:
    def __init__(self, prefixes: list[str], symbol_filter: SymbolGeneratorFilter, symbol_name_generator: SymbolNameGenerator):
    ...
    def generate_enum_case(self, enum_name: CompoundSymbolName, enum_original_name: str, node: c_ast.Enumerator) -> SwiftEnumCaseDecl:

        return SwiftEnumCaseDecl(
            self.symbol_name_generator.generate_enum_case(enum_name, enum_original_name, node.name),
    ...
    def generate(self, node: c_ast.Node) -> SwiftDecl | None:
        match node:
            ...
            case c_ast.Enum():
                ...
                if self.symbol_filter.should_gen_enum_case(node, decl):
                    return decl
